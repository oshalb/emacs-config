...
;; Run for existing frame (when not daemon)
(when (display-graphic-p)
  (oshal/setup-ui (selected-frame)))

;; Run for new frames (daemon + emacsclient)
(add-hook 'after-make-frame-functions #'oshal/setup-ui)

;;; +++++++++++++++++++++++++++++
;;; Centralized Auto backup/saves
;;; +++++++++++++++++++++++++++++
...
(use-package meow
  ...
  (meow-global-mode 1)
  :config
  (setq meow-cheatsheet-layout meow-cheatsheet-layout-qwerty) ; Qwerty Layout
...
(use-package treemacs
  ...
  (add-hook 'treemacs-mode-hook
            (lambda ()
	      ;; Disable line numbers + scroll bar
              (display-line-numbers-mode -1)
              (scroll-bar-mode -1)
	      ;; Skip treemacs window with C-x o
              (setq-local window-persistent-parameters
